...
class Proj5GUI( QMainWindow ):
    ...
    def solveClicked(self):
        ...
        results = self.solver.greedy(start_time = time.clock())
        # results = self.solver.defaultRandomTour( start_time = time.clock() )
        results = self.solver.branchAndBound(results['soln'], start_time = time.clock())
        print( results )
...
#
    ...
    ALGORITHMS = [ \
        ('Default',TSPSolver.defaultRandomTour), \
        ('Greedy',TSPSolver.greedy), \
        ('Branch and Bound',TSPSolver.branchAndBound), \
        ('Fancy',TSPSolver.fancy) \
    ]
    ...
    def initUI( self ):
        ...
        self.curSeed        = QLineEdit('0')
        self.curSeed.setFixedWidth(100)
        self.size           = QLineEdit('25')
        ...
        self.algDropDown.setCurrentIndex(0)
        self.algChanged(0) # to handle start state
